...
func (pc *HTTPConnector) RegenerateTLSCert() (err error) {
	addr := net.ParseIP(pc.responder.ListenIPAddress)
	pc.tlsCertificate, err = GetNewTLSCertificate(
		[]net.IP{addr},
		TLSCertExpiryHours,
	)
	if err == nil {
		logrus.Info("TLS certificate generated successfully.")
	} else {
		logrus.Error("TLS certificate generation failed: " + err.Error())
	}
	return
}
